...
// A sequence with one element and a sequence with two non-equal elements 
...
public:
    int wiggleMaxLength(vector<int>& nums) {
        ...
        {
            if (nums[i] == nums[i-1]) {++del;continue;}
            else if (nums[i] > nums[i-1]) {if (prev == 1) {++del;}prev = 1;}
            else {if (prev == 0) {++del;}prev = 0;}
        }
